        ...
        Box::pin(async move {
            ...
            Ok(BoundServer {
                ...
                run: Box::pin(async move {
                    ...
                    async move {
                        let (protocol, command, destination) = Socks5ServerProtocol::accept_no_auth(stream).await?.read_command().await?;
                        match command {
                            ...
                            Socks5Command::UDPAssociate => {
                                ...
                                let mut client = match destination {
                                    fast_socks5::util::target_addr::TargetAddr::Ip(addr) if addr.port() != 0 => Some(SocketAddr::new(peer.ip(), addr.port())),
                                    _ => None,
                                };
                                loop {
                                    tokio::select! {
                                        ...
                                        received = socket.recv_from(&mut buffer) => {
                                            ...
                                            if source.ip() != peer.ip() || client.is_some_and(|expected| expected != source) { continue; }
                                        ...
                                        response = driver.rx.recv() => {
                                            ...
                                            if let (Some(client), Ok(wire)) = (client, encode(response))
