...
The worker can reuse a TIME-WAIT tuple for a bare SYN whose sequence follows all bytes and FIN accepted from the previous peer. The fork checks this using wrapping TCP sequence comparisons and supplies a local ISN beyond the previous FIN, following RFC 6191 and RFC 9293 section 3.6.1. Duplicate SYNs preserve the old connection. Reuse waits for unread socket data and application EOF handling, then creates a separate connection with a new generation; old timers and readiness events cannot act on it.
...
UDP writers own their encoders and share atomic fragment identifiers. Unfragmented UDP is encoded directly into final packet storage. Fragmented UDP computes the checksum over its fixed header and borrowed payload separately, then copies each range directly into its final fragment without a full-datagram temporary buffer. When the Linux writer supports TCP GSO, smoltcp emits an aggregate and its segment size directly. A vectored TUN write passes the virtio header, IP/TCP header and immutable payload views. Eligible UDP datagrams similarly use header and payload vectors, with UDP GSO for adjacent equal-size datagrams belonging to the same flow. Oversized UDP datagrams retain the fragmentation path. Ordinary packets use vectored checksum-free virtio headers without GRO scratch storage.
...
After the daemon routes a TUN flow, eligible native TCP and UDP transports move to that flow's worker. The worker polls socket readiness, TCP state and packet I/O directly. TCP submits up to 32 received blocks in one vectored write, returns credit only for the accepted prefix, and retains the rest for socket readiness. It preserves half-close behavior and reports transferred byte counts to the session. Direct UDP opens scoped packet I/O without creating an outbound relay queue or handoff task. Other carriers can decline before network I/O and use the ordinary datagram path. UDP transfers its existing ingress queue before accepting new direct input, then releases that queue. Reply vectors grow on demand; idle associations do not reserve a full receive batch. Partial socket sends remove only the accepted prefix. Daemon routing, registration, carrier cancellation and idle expiry remain active. Encoded transports continue through the ordinary chunk or packet path.
